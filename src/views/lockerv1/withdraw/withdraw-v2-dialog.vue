          ...
          <template>
            <div class="pa-1 align-center flex-nowrap inputcolor r-outlined">
              ...
              <div class="d-flex align-center">
                <!-- <c-input
                  :value.sync="amountHuman"
                  placeholder="0.0"
                  @updateWhileFocussed="onHumanAmountChange"
                  class="pa-2 title font-weight-bold"
                ></c-input> -->
                <c-input
                  v-model:value="amountHuman"
                  placeholder="0.0"
                  @updateWhileFocussed="onHumanAmountChange"
                  class="pa-2 title font-weight-bold"
                ></c-input>
...
const withdrawableBalanceHuman = computed(() => {
  const amount = Utils.convertMicroDenomToDenom(lock.value.amount, decimals.value)
...
const onHumanAmountChange = (val) => {
  amount.value = Utils.convertDenomToMicroDenom(lock.value.amount, decimals.value)
}

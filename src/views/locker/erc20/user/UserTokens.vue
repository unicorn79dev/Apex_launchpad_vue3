...
export default {
  ...
  methods: {
    ...
    async getUserLockedTokens(_start, _count) {
      return await VestingPager.getUserLockedTokens(this.sEthers.coinbase, _start, _count);
    },
    async getUserLockedTokensLength() {
      const numTokens = await VestingPager.getUserLockedTokensLength(this.sEthers.coinbase);
    ...
    async refresh() {
      ...
      try {
        ...
        const addresses = await this.getUserLockedTokens(this.page * VESTABI.MAX_GETTER_LENGTH, VESTABI.MAX_GETTER_LENGTH);
        const tokens = [];
        for (const address of addresses) {
          const token = await ERC20.getERC20(address);
          tokens.push(token);
        }
        this.tokens = tokens;
